const NEURAL_LANG_PACK = {
    ...
    "msg_smith_threat": ["Minaccia rilevata nel Wormhole. Ho attivato il protocollo Firewall. Accesso negato al peer sospetto.", "Threat detected in the Wormhole. Firewall protocol activated. Access denied to suspicious peer."],
    "msg_smith_malware": ["Malware neutralizzato. Sto scansionando la Nebula in cerca di frammenti di codice corrotto. Integrità garantita.", "Malware neutralized. Scanning the Nebula for corrupted code fragments. Integrity guaranteed."],
    "msg_jabba_wealth": ["Ho-Ho-Ho... Il tuo Vault sta crescendo. Tanta conoscenza... tanto potere. Ho messo al sicuro i tuoi nodi più preziosi.", "Ho-Ho-Ho... Your Vault is growing. So much knowledge... so much power. I've secured your most precious nodes."],
    "msg_mission_complete": ["Missione Compiuta! I dati sono stati catalogati con saggezza.", "Mission Accomplished! Data has been cataloged with wisdom."],
    "msg_yoda_analyzing": ["Sto analizzando la struttura della Nebula... Ho individuato nodi interessanti, sto approfondendo le relazioni semantiche.", "Analyzing the Nebula structure... Interesting nodes identified, deepening semantic relations."],
    "msg_yoda_complete": ["Missione compiuta! Ho creato nuove galassie di conoscenza. Il Vault è ora più vasto.", "Mission accomplished! New knowledge galaxies created. The Vault is now vast."],
    "msg_distiller_pruning": ["Potatura semantica in corso... Sto eliminando archi obsoleti per ridurre l'entropia.", "Semantic pruning in progress... Eliminating obsolete arcs to reduce entropy."],
    "msg_janitron_cleaning": ["Waka-Waka! Sto ripulendo i frammenti orfani. Nodi inutilizzati eliminati dal kernel.", "Waka-Waka! Cleaning up orphan fragments. Unused nodes removed from kernel."],
    "msg_quantum_fusion": ["Poyo-Boop! Fusione nucleare dei cluster completata. Q-bert ha riallineato la densità semantica.", "Poyo-Boop! Nuclear cluster fusion complete. Q-bert has re-aligned semantic density."],
    "msg_sentinel_validating": ["Scudi attivi. Sto validando l'integrità dei nuovi nodi. Accesso concesso.", "Shields active. Validating node integrity. Access granted."],
    "msg_skywalker_foraging": ["Sto foraggiando dati dai confini esterni. Risultati in arrivo per il kernel.", "Foraging data from the outer rim. Results inbound for kernel."],
    "msg_pilot_returning": ["Rientro alla base. Mesh stabilizzata e nuovi dati ancorati alla Nebula.", "Returning to base. Mesh stabilized and new data anchored."],
    ...
    "msg_reaper_healed": ["Protocollo Medico completato. Ho riparato {n} frammenti di memoria e recuperato {mb} MB di spazio. Integrità cognitiva ripristinata.", "Medical Protocol complete. Repaired {n} memory fragments and reclaimed {mb} MB of space. Cognitive integrity restored."],
    "msg_kirby_discovery": ["Poyo! Ho trovato una nuova scia semantica! Sto tracciando il percorso...", "Poyo! Found a new semantic trail! Tracking the path..."],
    "msg_snake_discovery": ["Ssssss... sento l'odore di nuovi dati... Nuova scia semantica rilevata.", "Ssssss... I smell new data... New semantic trail detected."],
    "msg_jarjar_error": ["Ouch! Mee-sa fatto un pasticcio! Qualcosa è andato storto nella ricerca... Bomba bad!", "Ouch! Me-sa made a mess! Something went wrong in the search... Bomba bad!"],
    "msg_bridger_link": ["Ponte quantistico stabilito. Sto connettendo cluster distanti per una navigazione rapida.", "Quantum bridge established. Connecting distant clusters for rapid navigation."],
    "msg_chewie_buffer": ["RRRWWGGGHH! (Buffer svuotati. La Nebula respira meglio ora)", "RRRWWGGGHH! (Buffers cleared. The Nebula breathes better now)"],
    "msg_jabba_nodes": ["Muo-ha-ha... {n} nuovi schiavi di conoscenza per il mio palazzo. Eccellente.", "Muo-ha-ha... {n} new knowledge slaves for my palace. Excellent."],
    "msg_mando_path": ["Questa è la via. Ho tracciato {n} nuovi sentieri nella polvere stellare.", "This is the way. I've carved {n} new paths in the stardust."],
    "char_yoda": ["MAESTRO YODA", "MASTER YODA"],
    "char_skywalker_pilot": ["PILOTA STELLARE", "STARFIGHTER PILOT"],
    "char_chewie": ["CHEWBACCA (MANUTENZIONE)", "CHEWBACCA (MAINTENANCE)"],
    "char_jabba": ["JABBA (VAULT WEALTH)", "JABBA (VAULT WEALTH)"],
    "char_smith": ["AGENTE SMITH (FIREWALL)", "AGENT SMITH (FIREWALL)"],
    "char_jarjar": ["JAN JAR BINKS", "JAN JAR BINKS"],
    "char_skywalker": ["SKYWALKER (FS-77)", "SKYWALKER (FS-77)"],
    "char_snake": ["SNAKE (SN-008)", "SNAKE (SN-008)"],
    "char_janitron": ["JANITRON (PACMAN)", "JANITRON (PACMAN)"],
    "char_distiller": ["DISTILLATORE (INVADER)", "DISTILLER (INVADER)"],
    "char_quantum": ["Q-BERT (QA-101)", "Q-BERT (QA-101)"],
    "char_kirby": ["KIRBY (SY-009)", "KIRBY (SY-009)"],
    "char_r2d2": ["R2-D2 (MAGAZZINIERE)", "R2-D2 (WAREHOUSE)"],
    "char_mando": ["DIN DJARIN (DN-099)", "DIN DJARIN (DN-099)"],
    "char_sentinel": ["SENTINELLA (SE-007)", "SENTINEL (SE-007)"],
    "char_reaper": ["DR. REAPER (RP-001)", "DR. REAPER (RP-001)"],
    "char_bridger": ["EZRA BRIDGER (CB-003)", "EZRA BRIDGER (CB-003)"],
...
function applyLanguage() {
    ...
    if (window.syncGuideLanguage) window.syncGuideLanguage();
...
// 📟 [v9.3] Holographic Communication Link Logic
let currentHoloTab = 'unread';
let currentHoloFilter = 'ALL';
let hologramAutoDisplay = true;
let hologramHistory = [];
let hologramIsTyping = false;
let hologramTypingInterval = null;
...
window.hideHologram = () => {
    ...
    // 📟 [v10.6] Auto-Restore Archive if message was opened from history
    if (window._openedFromHistory) {
        window._openedFromHistory = false;
        const panel = document.getElementById('holo-history-panel');
...
window.filterHoloHistory = (agentId) => {
    currentHoloFilter = agentId;
    document.querySelectorAll('.agent-filter-icon').forEach(icon => icon.classList.remove('active'));
...
window.showHologram = (character, message, params = {}, duration = 13000) => {
    ...
    if (hologramTypingInterval) clearInterval(hologramTypingInterval);

    const isEn = localStorage.getItem('neuralvault_lang') === 'en';
    let realMessage = NEURAL_LANG_PACK[message] ? NEURAL_LANG_PACK[message][isEn ? 1 : 0] : message;
    
    // [v17.5] Dynamic Data Substitution
    if (duration && typeof duration === 'object') {
        // Handle case where 3rd arg is data object
        const data = duration;
        Object.keys(data).forEach(key => {
            realMessage = realMessage.split(`{${key}}`).join(data[key]);
        });
        duration = 13000;
    }
    ...
    const display = document.getElementById('hologram-display');
    const sender = document.getElementById('holo-sender');
    const msgEl = document.getElementById('holo-message');
    const avatar = document.getElementById('holo-avatar-container');
    
    if (!display || !msgEl) { hologramIsTyping = false; return; }

    // Character Mapping (Images & Styles)
    const chars = {
        'YODA': { img: '/static/img/holograms/yoda.png', color: '#4ade80', name: 'char_yoda', icon: 'fa-jedi' },
        'YODA_PILOT': { img: '/static/img/holograms/yoda_pilot.png', color: '#4ade80', name: 'char_yoda', icon: 'fa-jedi' },
        'PILOT': { img: '/static/img/holograms/skywalker.png', color: '#3b82f6', name: 'char_skywalker_pilot', icon: 'fa-fighter-jet' },
        'CHEWBACCA': { img: '/static/img/holograms/chewie.png', color: '#a16207', name: 'char_chewie', icon: 'fa-wrench' },
        'JABBA': { img: '/static/img/holograms/jabba_wireframe.png', color: '#00ff41', name: 'char_jabba', icon: 'fa-coins' },
        'SMITH': { img: '/static/img/holograms/smith.png', color: '#00ff41', name: 'char_smith', icon: 'fa-user-secret' },
        'JARJAR': { img: '/static/img/holograms/jarjar.png', color: '#00ff41', name: 'char_jarjar', icon: 'fa-surprise' },
        'SKYWALKER': { img: '/static/img/holograms/skywalker.png', color: '#ef4444', name: 'char_skywalker', icon: 'fa-rocket' },
        'SNAKE': { img: '/static/img/holograms/snake_wireframe.png', color: '#00ff41', name: 'char_snake', icon: 'fa-worm' },
        'JANITRON': { img: '/static/img/holograms/janitron.png', color: '#facc15', name: 'char_janitron', icon: 'fa-ghost' },
        'DISTILLER': { img: '/static/img/holograms/distiller.png', color: '#f87171', name: 'char_distiller', icon: 'fa-wine-glass' },
        'QUANTUM': { img: '/static/img/holograms/qbert_wireframe.png', color: '#00ff41', name: 'char_quantum', icon: 'fa-cube' },
        'QBERT': { img: '/static/img/holograms/qbert_wireframe.png', color: '#00ff41', name: 'char_quantum', icon: 'fa-cube' },
        'BRIDGER': { img: '/static/img/holograms/bridger.png', color: '#00ff41', name: 'char_bridger', icon: 'fa-project-diagram' },
        'JABBA_LAUGH': { img: '/static/img/holograms/jabba_front.png', color: '#00ff41', name: 'char_jabba', icon: 'fa-grin-tongue-wink' },
        'KIRBY': { img: '/static/img/holograms/kirby_wireframe.png', color: '#00ff41', name: 'char_kirby', icon: 'fa-music' },
        'SYNTH': { img: '/static/img/holograms/kirby_wireframe.png', color: '#00ff41', name: 'char_kirby', icon: 'fa-music' },
        'R2D2': { img: '/static/img/holograms/r2d2.png', color: '#3b82f6', name: 'char_r2d2', icon: 'fa-robot' },
        'MANDALORIAN': { img: '/static/img/holograms/mando_wireframe.png', color: '#00ff41', name: 'char_mando', icon: 'fa-user-shield' },
        'SENTINEL': { img: '/static/img/holograms/sentinel.png', color: '#3b82f6', name: 'char_sentinel', icon: 'fa-user-shield' },
        'REAPER': { img: '/static/img/holograms/reaper_mario.png', color: '#00ff41', name: 'char_reaper', icon: 'fa-microscope' }
    };

    const c = chars[character] || chars['PILOT'];
    
    // Log to History (Only if new, not if re-opening from archive)
    if (!window._openedFromHistory) {
        const logItem = {
            charId: character,
            character: c.name,
            message: realMessage,
            color: c.color,
            icon: c.icon || 'fa-user-astronaut',
            time: new Date().toLocaleTimeString(),
            isRead: false
        };
        hologramHistory.unshift(logItem);
        renderHologramHistory();
    }

    if (!hologramAutoDisplay && !window._manualHoloTrigger) {
        hologramIsTyping = false;
        return;
    }
    window._manualHoloTrigger = false; 

    // Setup UI
    const charName = NEURAL_LANG_PACK[c.name] ? NEURAL_LANG_PACK[c.name][isEn ? 1 : 0] : c.name;
    display.style.color = c.color; 
    sender.innerText = `COMM_LINK: ${charName}`;
    sender.style.color = c.color;
    
    if (c.img) {
        avatar.innerHTML = `<div style="width:100%; height:100%; background: url('${c.img}') center/cover no-repeat; ${c.style || ''} filter: sepia(1) saturate(5) hue-rotate(90deg) brightness(1.2) drop-shadow(0 0 10px ${c.color});"></div>`;
    } else {
        avatar.innerHTML = `<i class="fas ${c.icon} fa-4x" style="color: ${c.color}; filter: drop-shadow(0 0 10px ${c.color});"></i>`;
    }
    display.style.borderColor = c.color;
    display.style.boxShadow = `0 0 40px ${c.color}66`;
    
    display.classList.add('active');
    display.classList.add('holo-glitch-active');
    setTimeout(() => display.classList.remove('holo-glitch-active'), 500);

    msgEl.innerHTML = "";
    let i = 0;
    
    hologramTypingInterval = setInterval(() => {
        if (i < realMessage.length) {
            msgEl.innerHTML += realMessage.charAt(i);
            i++;
        } else {
            clearInterval(hologramTypingInterval);
            hologramIsTyping = false;
        }
    }, 25);

    if (window.hologramTimeout) clearTimeout(window.hologramTimeout);
    window.hologramTimeout = setTimeout(() => {
        window.hideHologram();
    }, duration);
};

function renderHologramHistory() {
    const list = document.getElementById('holo-history-list');
    const unreadCount = document.getElementById('unread-count');
    const filterContainer = document.getElementById('holo-agent-filters');
    if (!list) return;

    // Update unread badge
    const unreadItems = hologramHistory.filter(item => !item.isRead);
    if (unreadCount) {
        unreadCount.innerText = unreadItems.length;
        unreadCount.style.display = unreadItems.length > 0 ? 'block' : 'none';
    }

    // Dynamic Filter Icons
    if (filterContainer) {
        const activeAgents = [...new Set(hologramHistory.map(h => h.charId))];
        let filterHtml = `<div class="agent-filter-icon ${currentHoloFilter === 'ALL' ? 'active' : ''}" title="ALL AGENTS" onclick="filterHoloHistory('ALL')"> <i class="fas fa-users"></i> </div>`;
        
        const agentIcons = {
            'YODA': 'fa-jedi', 'SKYWALKER': 'fa-rocket', 'SMITH': 'fa-user-secret', 
            'KIRBY': 'fa-music', 'R2D2': 'fa-robot', 'JANITRON': 'fa-ghost', 
            'DISTILLER': 'fa-wine-glass', 'SNAKE': 'fa-worm', 'SENTINEL': 'fa-shield-halved'
        };

        activeAgents.forEach(aid => {
            const icon = agentIcons[aid] || 'fa-user-astronaut';
            filterHtml += `<div class="agent-filter-icon ${currentHoloFilter === aid ? 'active' : ''}" title="${aid}" onclick="filterHoloHistory('${aid}')"> <i class="fas ${icon}"></i> </div>`;
        });
        filterContainer.innerHTML = filterHtml;
    }

    let filtered = currentHoloTab === 'unread' ? unreadItems : hologramHistory;
    if (currentHoloFilter !== 'ALL') {
        filtered = filtered.filter(item => item.charId === currentHoloFilter);
    }

    if (filtered.length === 0) {
        list.innerHTML = `<div style="text-align:center; padding:40px; color:rgba(0,255,65,0.3); font-style:italic;">NESSUN MESSAGGIO</div>`;
        return;
    }

    list.innerHTML = filtered.map((item, index) => {
        const realIndex = hologramHistory.indexOf(item);
        return `
            <div class="history-item ${item.isRead ? 'is-read' : ''}" onclick="reShowHologram(${realIndex})">
                <div class="history-item-header">
                    <span class="history-item-char" style="color: ${item.color}">${item.character}</span>
...
window.toggleAutoHolograms = (enabled) => {
    hologramAutoDisplay = enabled;
    console.log("📟 [Hologram] Auto-display set to:", enabled);
};

window.reShowHologram = (index) => {
    const item = hologramHistory[index];
    if (!item) return;
    
    item.isRead = true; // Mark as read when clicked
    
    // 📟 [v10.6] Archive UX: Close archive, show message, set restore flag
    const panel = document.getElementById('holo-history-panel');
    if (panel) panel.classList.remove('active');
    
    window._openedFromHistory = true; 
    window._manualHoloTrigger = true; 
    window.showHologram(item.charId, item.message);
    renderHologramHistory();
};

// Sidebar Interaction
document.addEventListener('DOMContentLoaded', () => {
    const toggle = document.getElementById('holo-history-toggle');
    const close = document.getElementById('holo-history-close');
    const panel = document.getElementById('holo-history-panel');
...
/**
 * 🛰️ [DN-099] Mandalorian Celestial Scanner
 */
function spawnMandoScanner(group) {
    if (!group) return;
    const beamGeom = new THREE.CylinderGeometry(500, 80000, 3000000, 32, 1, true);
    ...
    beam.position.z = 1500000;
    group.add(beam);
    setTimeout(() => { group.remove(beam); }, 100);
}

window.spawnYodaLaser = (start, end) => { if(typeof spawnYodaBullet === 'function') spawnYodaBullet(start, end); };
...
const AGENT_PROFILES = {
    ...
    'RP-001': { title: "RP-001 DR. REAPER", role: "TOMBSTONE REGENERATION", desc: "Cura le lapidi lasciate dalle eliminazioni, compattando il DB.", example: "Ripara i settori di memoria post-chirurgia janitroniale." },
...
async function registerCustomModel(source = 'settings') {
    const prefix = source === 'hub' ? 'hub-' : 'settings-';
    const nameEl = document.getElementById(prefix + 'custom-model-name');
    const providerEl = document.getElementById(prefix + 'custom-model-provider');
    const pathEl = document.getElementById(prefix + 'custom-model-path');

    if (!nameEl || !pathEl) {
        log("❌ FORGE: Elementi UI non trovati.", "#ef4444");
        return;
    }
    ...
    try {
        ...
        if (result.success) {
            ...
            // Re-sync UI
            if (typeof refreshModels === 'function') await refreshModels();
            if (typeof renderModelHubTable === 'function') await renderModelHubTable();
...
window.toggleTheme = () => {
    ...
    // [v13.6] Forza aggiornamento colori archi interni
    if (typeof vaultPoints !== 'undefined' && vaultPoints.length > 0) {
    ...
    const interval = setInterval(async () => {
        try {
            ...
            if (status) {
                ...
                // [v18.5] Support dual status indicators
                ['download-install-status-text', 'uplink-install-status-text'].forEach(id => {
                    const el = document.getElementById(id);
                    if (!el) return;

                    el.innerText = `SYNAPSE ${status.status.toUpperCase()}: ${perc}%`;
                    if (status.status === 'success') {
                        el.innerText = "SYNAPSE INTEGRATA CON SUCCESSO!";
                        el.style.color = "#4ade80";
                    } else if (status.status === 'error') {
                        el.innerText = `ERRORE: ${status.message}`;
...
window.toggleFollow = (agentId) => {
    // 🔍 [v10.6] Enhanced Agent Lookup System
    const mapping = {
        'JA-001': window.janitronGroup, 
        'DI-007': window.distillerGroup, 
        'RP-001': window.reaperGroup,
        'SN-008': window.snakeGroup, 
        'QA-101': window.quantumGroup, 
        'SE-007': window.sentinelGroup,
        'YO-001': window.yodaGroup, 
        'R2-D2': window.r2d2Group, 
        'DN-099': window.mandalorianGroup,
    ...
    if (window.smithFleetGroups && Object.values(window.smithFleetGroups).length > 0) {
        mapping['AG-001'] = Object.values(window.smithFleetGroups)[0];
    }
    ...
    // Visual feedback in all agent bars
    document.querySelectorAll('.agent-mission-item, .agent-card-v2').forEach(el => el.classList.remove('followed-agent'));

    if (window.followedAgent === target || !target) {
        window.followedAgent = null;
        if (window.log) window.log(`📡 CAMERA: Swarm Orbit Resumed`, "#3b82f6");
    } else {
        window.followedAgent = target;
        if (window.log) window.log(`📡 CAMERA: Hooked onto ${agentId}`, "#10b981");
        
        // Highlight active card in sidebar
        const hudId = {
            'DI-007': 'distiller-hud-icon', 'JA-001': 'janitron-hud-icon', 'RP-001': 'reaper-hud-icon',
        ...
        if (hudId) {
            ...
            if (el) el.classList.add('followed-agent');
        }
    }
};
...
// --- Toggles moved to globals.js ---

function initCharts() {
    const growthCanvas = document.getElementById('growth-chart');
    if (growthCanvas) {
        const existing = Chart.getChart(growthCanvas);
    ...
    if (densityCanvas) {
        const existing = Chart.getChart(densityCanvas);
        if (existing) existing.destroy();
        const densityCtx = densityCanvas.getContext('2d');
        window.densityChart = new Chart(densityCtx, {
            type: 'bar',
            data: {
                labels: ['V-MESH', 'RAG-CORE', 'SENTINEL', 'SYNP-X'],
                datasets: [{
                    label: 'DENSITÀ NEURALE',
                    data: [65, 82, 45, 93],
    ...
    if (impactCanvas) {
        const existing = Chart.getChart(impactCanvas);
        if (existing) existing.destroy();
...
window.renderAgentGrid = (labData) => {
    ...
    grid.innerHTML = Object.keys(agents).map(id => {
        ...
        const statusColor = status === 'active' || status === 'operating' || status.includes('monitoring') ? '#4ade80' : 
...
window.toggleLayer = (layer) => {
    ...
    const agents = [janitronGroup, distillerGroup, reaperGroup, snakeGroup, quantumGroup, sentinelGroup, synthGroup, bridgerGroup, skywalkerGroup, yodaGroup, mandalorianGroup];
    if (layer === 'agents') { 
